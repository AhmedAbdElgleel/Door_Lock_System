...
/*NOTE:
 * 		UBRRH shared the same I/O memory Location(Mapped on same location) as USCRC Register so
 * 		URSEL_BIT must be zero when writing UBRRH and 1 when access UCSRC*/
typedef union
{
	uint8 Byte;
	struct
...
/******************************************************************************************
 * Registers definitions for each type struct for UART in the memory mapped Registers   *
 ******************************************************************************************/
#define UART_UDR_REG   		(*((volatile Uart_UDR_Type*   const)0x002C))
#define UART_UCSRA_REG      (*((volatile Uart_UCSRA_Type* const)0x002B))
#define UART_UCSRB_REG      (*((volatile Uart_UCSRB_Type* const)0x002A))
#define UART_UCSRC_REG      (*((volatile Uart_UCSRC_Type* const)0x0040))
#define UART_UBRRL_REG      (*((volatile Uart_UBRRL_Type* const)0x0029))
#define UART_UBRRH_REG      (*((volatile Uart_UBRRH_Type* const)0x0040))
